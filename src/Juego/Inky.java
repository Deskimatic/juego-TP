...
public class Inky extends Enemigo {
    ...
    private static Image cargarImagen(){
        ...
        return new ImageIcon(
                Inky.class.getResource(
                        "/Juego/recursos/scaredGhost.png"
                )
        ).getImage();
    ...
    public void mover(Tablero tablero, Jugador jugador){
        ...
        int distancia = Math.abs(
                blinky.getFila() - jugador.getFila()
        ) +
                Math.abs(
                        blinky.getColumna() - jugador.getColumna()
                );

        int nuevaFila = fila;
        int nuevaColumna = columna;

        // Si Blinky está cerca de Pac-Man,
        // Inky se mueve aleatoriamente.
